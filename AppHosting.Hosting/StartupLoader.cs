...
namespace AppHosting.Hosting
{
    internal class StartupLoader
    {
        ...
        public static StartupMethods LoadMethods(IServiceProvider hostingServiceProvider, Type startupType, string environmentName)
        {
            ...
            var registerRoutesMethod = FindRegisterRoutesDelegate(startupType, environmentName);
            var configureContainerMethod = FindConfigureContainerDelegate(startupType, environmentName);

            object instance = default;
            if (servicesMethod != default && !servicesMethod.MethodInfo.IsStatic)
            {
            ...
            var type = configureContainerMethod.MethodInfo != default ? configureContainerMethod.GetContainerType() : typeof(object);
            ...
            return new StartupMethods(instance, registerRoutesMethod.Build(instance), builder.Build());
        }
        ...
        internal static RegisterRoutesBuilder FindRegisterRoutesDelegate(Type startupType, string environmentName)
        {
            var registerRoutesMethod = FindMethod(startupType, "Register{0}Routes", environmentName, typeof(void), required: false);
            return new RegisterRoutesBuilder(registerRoutesMethod);
        }

        private static MethodInfo FindMethod(Type startupType, string methodName, string environmentName, Type returnType = default, bool required = true)
        {
            ...
            if (returnType != default && methodInfo.ReturnType != returnType)
            {
                if (required)
                {
        ...
        private class ConfigureServicesDelegateBuilder<TContainerBuilder> : ConfigureServicesDelegateBuilder
        {
                ...
                Action<object> configureContainerCallback)
            {
                ...
                IServiceProvider ConfigureServicesWithContainerConfiguration(IServiceCollection services)
                {
                    ...
                    if (applicationServiceProvider != default)
                    {
                        return applicationServiceProvider;
                    }

                    // If there's a ConfigureContainer method
                    if (ConfigureContainerBuilder.MethodInfo != default)
                    {
...
#pragma warning restore CS0612 // Type or member is obsolete
                    ...
                    // We return default so that the host here builds the container (same result as void ConfigureServices(IServiceCollection services);
                    return default;

                    void InvokeStartup(IServiceCollection serviceCollection)
                    {
                        var result = startup(serviceCollection);
                        if (filters.Length > 0 && result != default)
                        {
